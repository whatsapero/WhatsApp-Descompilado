...
public abstract class AbstractBox implements Box {
    ...
    private synchronized void readContent() {
        try {
            if (!this.isRead) {
                ...
                this.content = this.fileChannel.map(MapMode.READ_ONLY, this.contentStartPosition, this.memMapSize);
                this.isRead = true;
            }
        } catch (IOException e) {
    ...
    public void getBox(WritableByteChannel os) throws IOException {
        ...
        if (!this.isRead) {
            ...
            header = ByteBuffer.allocate(i);
            getHeader(header);
            os.write((ByteBuffer) header.rewind());
            this.fileChannel.transferTo(this.contentStartPosition, this.memMapSize, os);
    ...
    public long getSize() {
        int i;
        int i2 = 0;
        long size = this.isRead ? this.isParsed ? getContentSize() : (long) this.content.limit() : this.memMapSize;
        if (size >= 4294967288L) {
            i = 8;
        } else {
            i = 0;
