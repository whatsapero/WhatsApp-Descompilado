...
public class AppendTrack extends AbstractTrack {
    ...
    private SampleEntry mergeSampleEntry(SampleEntry se1, SampleEntry se2) {
        ...
        return ((se1 instanceof AudioSampleEntry) && (se2 instanceof AudioSampleEntry)) ? mergeAudioSampleEntries((AudioSampleEntry) se1, (AudioSampleEntry) se2) : null;
    }
    ...
    public List<Entry> getDecodingTimeEntries() {
        ...
        for (long[] list : lists) {
            for (long nuDecodingTime : (long[]) r11.next()) {
    ...
    public List<CompositionTimeToSample.Entry> getCompositionTimeEntries() {
        ...
        for (int[] list : lists) {
            for (int compositionTime : (int[]) r9.next()) {
                if (compositionTimeEntries.isEmpty() || ((CompositionTimeToSample.Entry) compositionTimeEntries.getLast()).getOffset() != compositionTime) {
                    compositionTimeEntries.add(new CompositionTimeToSample.Entry(1, compositionTime));
                } else {
                    CompositionTimeToSample.Entry e = (CompositionTimeToSample.Entry) compositionTimeEntries.getLast();
                    e.setCount(e.getCount() + 1);
                }
            }
        }
